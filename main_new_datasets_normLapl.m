...
for i=1:length(K)
    [L_tetra{i}, D_tetra{i}] = graph_laplacian_norm(W_tetra{i});
    [L_chainlink{i}, D_chainlink{i}] = graph_laplacian_norm(W_chainlink{i});
    [L_atom{i}, D_atom{i}] = graph_laplacian_norm(W_atom{i});
...
for i = 1:length(K)
    ...
    U_atom{i} = U_atom{i}(:, 1:5); %matrix is badly conditioned, computation of eigs is inaccurate
end
